      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
        ...
        <div class="lg:col-span-2">
          <a href="#" class="flex items-center gap-3" @click="scrollTo($event, '#')">
            <img :src="churchData.logo" :alt="churchData.name" class="h-8 w-auto" />
        ...
        <div>
          ...
          <address class="mt-4 text-slate-600 not-italic leading-relaxed">
            {{ churchData.digital_address }}<br />
            {{ churchData.address }}, {{ churchData.city }}, {{ churchData.country }}
          </address>
        ...
        <div>
          ...
          <p class="mt-6 text-slate-600">{{ churchData.email }}</p>
        </div>
      </div>

      <!-- Copyright -->
      <div class="mt-12 pt-8 border-t border-slate-200 text-center text-slate-500 text-sm">
        <p>&copy; {{ new Date().getFullYear() }} {{ churchData.name }}. All rights reserved.</p>
      </div>
    </div>
  </footer>
</template>
