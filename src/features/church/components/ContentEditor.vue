...
const beliefs       = computed(() => Array.isArray(formData.value.beliefs) ? formData.value.beliefs : [])
const values        = computed(() => Array.isArray(formData.value.values)  ? formData.value.values  : [])
const ministryItems = computed(() => Array.isArray(formData.value.items)   ? formData.value.items   : [])
    ...
    <CardContent class="space-y-6">
        ...
        <div class="space-y-4">
          ...
          <div class="grid gap-4">
            ...
            <div class="space-y-2">
              ...
              <Textarea v-model="formData.verse_text" rows="3" placeholder="Enter verse text" class="resize-none" />
            </div>
          </div>
        </div>
      </template>
      ...
      <template v-else-if="section === 'about'">
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            <Label>Description <span class="text-xs text-muted-foreground ml-1">(supports rich text)</span></Label>
            <RichTextEditor
          ...
          <div class="space-y-3">
            ...
            <div class="space-y-2">
              ...
              <p v-if="values.length === 0" class="text-sm text-muted-foreground">No values added yet.</p>
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div class="space-y-2">
              ...
              <p v-if="beliefs.length === 0" class="text-sm text-muted-foreground">No beliefs added yet.</p>
            </div>
          </div>
        </div>
      </template>
      ...
      <template v-else-if="section === 'quote'">
        <div class="space-y-4">
          <div class="space-y-2">
            <Label>Background Image <Badge variant="outline" class="ml-1 text-xs">Optional</Badge></Label>
            <FileUpload
              :model-value="uploadedFiles.background_image"
        ...
        <div class="space-y-4">
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="space-y-2">
              ...
              <Input v-model="formData.description" placeholder="Discover meaningful ways to connect…" />
            </div>
          </div>
          <Separator />
          <div class="space-y-3">
            ...
            >
              ...
              <div class="grid gap-3 sm:grid-cols-2">
                ...
                <div class="space-y-1.5">
                  <Label class="text-xs">Icon <Badge variant="outline" class="ml-1 text-xs">Optional</Badge></Label>
                  <Input v-model="item.icon" placeholder="e.g. zap, heart, users" />
                </div>
              </div>
              <div class="space-y-1.5">
                <Label class="text-xs">Description <span class="text-muted-foreground">(rich text)</span></Label>
                <RichTextEditor
      ...
      <template v-else-if="section === 'featured_sermon'">
        <div class="space-y-4">
          ...
          <div class="grid gap-4">
            ...
            <div class="space-y-2">
              <Label>Description <span class="text-xs text-muted-foreground ml-1">(rich text)</span></Label>
              <RichTextEditor
                v-model="formData.description"
            ...
            <div class="space-y-2">
              <Label>Video URL <Badge variant="outline" class="ml-1 text-xs">Optional</Badge></Label>
              <Input v-model="formData.video_url" type="url" placeholder="https://youtube.com/watch?v=…" />
            </div>
          </div>
        </div>
      </template>
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              ...
              <Button
                size="sm" variant="outline"
                @click="addItem('items', { title: '', excerpt: '', date: '', image: null, link: '' })"
              >
            ...
            >
              ...
              <div class="space-y-1.5">
                <Label class="text-xs">Excerpt <span class="text-muted-foreground">(rich text)</span></Label>
                <RichTextEditor
              ...
              <div class="space-y-1.5">
                <Label class="text-xs">Link <Badge variant="outline" class="ml-1 text-xs">Optional</Badge></Label>
        ...
        <div class="space-y-4">
          <div class="grid gap-4">
            <div class="space-y-2">
              ...
              <Input v-model="formData.facebook" type="url" placeholder="https://facebook.com/yourpage" />
            </div>
            <div class="space-y-2">
              <Label>Instagram</Label>
              <Input v-model="formData.instagram" type="url" placeholder="https://instagram.com/yourhandle" />
            </div>
            <div class="space-y-2">
              <Label>YouTube</Label>
              <Input v-model="formData.youtube" type="url" placeholder="https://youtube.com/@yourchannel" />
            </div>
            <div class="space-y-2">
